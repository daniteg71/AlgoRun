...
| Pairwise candidate generation | ALL domain/range-compatible entity pairs per relation, no trigger (deliberate over-generation: on train, 502 pos / 2080 neg; ceiling recall 0.39 vs 0.14 of the trigger baseline) | `src/algorun/refinery.py` (`extract_candidates`) | Course Block 14 Architecture A ("pairwise" option); measured 2026-07-06 |
| Supervised binary relation validator | Fine-tuned DistilBERT-base-uncased, (sentence, verbalized candidate) → VALID/INVALID; 3 epochs, AdamW lr 5e-5, batch 16, on Apple MPS (~2 min); val accuracy 0.99. **Measured on held-out test.jsonl: baseline F1 0.241 → generator+validator F1 0.478** (P 1.00→0.903, R 0.137→0.325); per tier: long_distance 0.00→0.47 (the validator reads the whole sentence — no token window), implicit 0.16→0.57, explicit 0.42→0.58, nested 0.27→0.44 | `src/algorun/validator.py`, `tests/test_validator.py` | Sanh et al. (2019) arXiv:1910.01108 DistilBERT; course "Generator→Validator" architectural shift |
...
| Lightweight relation validator | Logistic regression (scikit-learn) on hand-made features (predicate one-hot, trigger presence, trigger↔endpoint token distances, subject↔object distance, canonical flags), `class_weight="balanced"`. **Measured on held-out test.jsonl: F1 0.429** (P 0.634, R 0.325) vs DistilBERT 0.478 — ~90% of the benefit at ~0 cost (<1 s CPU, ~KB). No torch/GPU. | `src/algorun/light_validator.py`, `tests/test_light_validator.py` | classic ML baseline; the right-sizing argument (see notes) |
| Supervised binary relation validator (RoBERTa variant) | RoBERTa-base, same script (`train --arch roberta`). **Trained on Colab (T4): COLLAPSED to the majority class** — val_accuracy stuck at 0.786 (= the negative fraction), test F1 0.00. Cause: lr 5e-5 too high for RoBERTa + unhandled 19/81 imbalance → it learned to always answer INVALID. Fixable (lr 1–2e-5 + class weighting) but confirms RoBERTa is heavier *and* more fragile than DistilBERT for this task | M4 (second architecture) | Liu et al. (2019) arXiv:1907.11692 |
